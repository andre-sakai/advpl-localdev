...
User Function MTaprova(cPedido)
	...
	While TRB2->(!eof())
		...
		If Select("QUCOM") <> 0
				...
				QUCOM->(dbCloseArea())
		EndIf				
		cQuery := " SELECT  D1_FILIAL, D1_COD,D1_SERIE, D1_DOC, D1_VUNIT, D1_DTDIGIT, D1_FORNECE, D1_LOJA, SD1.R_E_C_N_O_ "                
		...
		cQuery += "               WHERE  SD1.D_E_L_E_T_ = ' ' and SF4.D_E_L_E_T_ = ' ' and SB1.D_E_L_E_T_ = ' '  and SD1.D1_TIPO = 'N'  and SF4.F4_DUPLIC = 'S' and D1_COD = '"+TRB2->C7_PRODUTO+"' "
		...
		cQuery += "               Inner Join "+RetSqlName("SF4")+" SF4 on SF4.F4_CODIGO = SD1.D1_TES "
		cQuery += "               Inner Join "+RetSqlName("SB1")+" SB1 on SB1.B1_COD    = SD1.D1_COD "
		cQuery += "               WHERE  SD1.D_E_L_E_T_ = ' ' and SD1.D1_DTDIGIT < '" + DTOS(dDtDigit) + "' and SF4.D_E_L_E_T_ = ' ' and SB1.D_E_L_E_T_ = ' '  and SD1.D1_TIPO = 'N'  and SF4.F4_DUPLIC = 'S' and D1_COD = '"+TRB2->C7_PRODUTO+"' "
		cQuery += " ORDER BY SD1.R_E_C_N_O_ DESC  "		
		TCQuery cQuery NEW ALIAS "QUCOM"   				
		dbSelectArea("QUCOM") 
		if !empty(QUCOM->d1_doc)						
		...
		cQuery += "               Inner Join "+RetSqlName("SF4")+" SF4 on SF4.F4_CODIGO = SD1.D1_TES "
		cQuery += "               Inner Join "+RetSqlName("SB1")+" SB1 on SB1.B1_COD    = SD1.D1_COD "
		cQuery += "               WHERE  SD1.D_E_L_E_T_ = ' ' and SD1.D1_DTDIGIT < '" + DTOS(dDtDigi2) + "' and SF4.D_E_L_E_T_ = ' ' and SB1.D_E_L_E_T_ = ' '  and SD1.D1_TIPO = 'N'  and SF4.F4_DUPLIC = 'S' and D1_COD = '"+TRB2->C7_PRODUTO+"' "
		cQuery += " ORDER BY SD1.R_E_C_N_O_ DESC  "		
		TCQuery cQuery NEW ALIAS "QUCOM"   				
		dbSelectArea("QUCOM") 
		if !empty(QUCOM->d1_doc)						
	   	...
	   	if nPrcUC  > 0 .and. nPrcUC < TRB2->C7_PRECO
			...
			oHtml:ValByName("preco",transform(TRB2->C7_PRECO, "@E 999,999,999.99"))  			
	
		   	oHtml:ValByName("UltCompra",transform(nPrcUC, "@E 999,999,999.99"))   
		  	oHtml:ValByName("DtDigit"  ,dDtDigit) 
			oHtml:ValByName("DocUltComp"  ,cDoc)
			
			if nPrcUC2 > 0
				oHtml:ValByName("UltCompra2",transform(nPrcUC2, "@E 999,999,999.99") )   
			  	oHtml:ValByName("DtDigit2"  ,dDtDigi2) 
				oHtml:ValByName("DocUltComp2"  ,cDoc2)
			endif
			
			if nPrcUC3 > 0
				oHtml:ValByName("UltCompra3",transform(nPrcUC3, "@E 999,999,999.99") )   
