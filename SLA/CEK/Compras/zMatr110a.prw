...
User Function zMatr110a()


Private cNumPed		 	:= SC7->C7_NUM
Private cForn		   	 	:= SC7->C7_FORNECE
Private cForLoja		 	:= SC7->C7_LOJA
Private dEmisa       	:= SC7->C7_EMISSAO
...
//oPrn:say (nLin,2730,Alltrim(Posicione("SE4",1,xFilial("SE4")+(_cAlias)->C7_COND, "E4_DESCRI")), oFont08)
...
oPrn:say (nLin,2700,"Condição de Pagamento: " + Alltrim(Posicione("SE4",1,xFilial("SE4")+(_cAlias)->C7_COND, "E4_DESCRI")), oFont08I) //"Condição de Pagamento:"
...
cQuery += " SC7.C7_TOTAL, SC7.C7_EMISSAO, SC7.C7_DATPRF, SC7.C7_IPI, SC7.C7_DESC1,SC7.C7_COND,"
...
cQuery += " AND SC7.C7_LOJA    = '"+(cForLoja)+"' "
cQuery += " AND SC7.C7_NUM    = '"+(cNumPed)+"' "
cQuery += " AND SC7.C7_EMISSAO =  '"+Dtos(dEmisa)+"' "
