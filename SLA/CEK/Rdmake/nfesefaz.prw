...
Private ddtsai		 // customizado C&K
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						EndIF 
     					...
     					//CUSTOMIZADO C&K
			            ddtsai := posicione("SC5", 1, XFILIAL("SC5") +  (cAliasSD2)->D2_PEDIDO , "C5_ZDTSAI" )			            
			            //FIM CUSTOMIZAÇÃO					
...
If !Empty(aNota)
	...
	cString += NfeIde(@cNFe,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd,aExp,aComb, ddtsai)//customizado ddtsai))
...
Static Function NfeIde(cChave,aNota,cNatOper,aDupl,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cIndPres,aDest,aProd,aExp,aComb, ddtsai)//customizado ddtsai))
...
If cVeramb >= "3.10"
	...
	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(ddtsai)+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
Else	
