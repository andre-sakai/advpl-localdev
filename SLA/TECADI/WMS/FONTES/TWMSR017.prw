...
User Function TWMSR017()
	...
	// busca Pedidos.
	_cQuery := "SELECT Z05_CLIENT ,Z05_LOJA,Z05_PROCES,Z04_NF,F1_DTDIGIT,Z2_DOCUMEN,ZZ_CNTR01,ZZ_PLACA1,Z42_PROD,B1_DESC,(Z42_QTDORI - Z42_QTDCON),Z42_QTDORI, Z42_QTDCON,Z35_DESCRI,Z41_CODIGO, Z42_LOTE, D1_LOTECTL "
	...
	*/
	...
	If _lAvaria
		...
		For _nBox:= 1 To Len(_aRetSQL)
			_aLinhaR := _aRetSQL[_nBox]
			If (_aRetSQL[_nBox][12] <> _aRetSQL[_nBox][13])
				oPrint:Say(_nLinBox1 + 5 ,0060 ,_aRetSQL[_nBox][09]               ,oFont11)
				oPrint:Say(_nLinBox1 + 5 ,0380 ,alltrim(_aRetSQL[_nBox][10]) + iif(!empty(_aLinhaR[16]),' / '+alltrim(_aLinhaR[16]),iif(!empty(_aLinhaR[17]),' / '+alltrim(_aLinhaR[17]),''))               ,oFont11)
