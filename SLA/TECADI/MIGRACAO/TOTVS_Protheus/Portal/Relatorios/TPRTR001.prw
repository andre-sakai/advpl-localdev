...
!                  ! e travado somente para o cliente logado no portal       !
+------------------+---------------------------------------------------------+
!Autor             ! Luiz Poleza                         ! Data    ! 10/2018 !
+------------------+---------------------------------------------------------+
!Observacoes       ! - Customizaçao do relatório padrão Totvs para exibir a  !
!                  ! coluna customizada "Pedido cliente", pois o relatório   !
!                  ! original não possui seção com vínculo na tabela de      !
!                  ! pedidos de venda                                        !
!                  ! - Restringe a seleção apenas para produtos do cliente   !
!                  ! logado/configurado no portal (usuário)                  !
+------------------+--------------------------------------------------------*/


User Function TPRTR001()
	Local oReport
...
Static Function ReportDef()
	...
	//³                                                                        ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oReport:= TReport():New("MATR910",STR0003,"TPRTR001", {|oReport| ReportPrint(oReport,oSection1)},STR0001+" "+STR0002) //"Kardex Fisico-Financeiro (DIA)"##"Este programa emitir  uma rela‡„o com as movimenta‡”es"##"dos produtos Selecionados,Ordenados por Dia."
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof()) .And. lImpLivro
		...
		While !oReport:Cancel() .And. !(cAliasTop)->(Eof()) .And. (cAliasTop)->PRODUTO = cProdAnt
			...
			Do Case
				...
				//Gustavo, SLA, inclusão da numeração do pedido do cliente
				cQuery := ""
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³Matr910R5 ³ Autor ³ Paulo Boschetti       ³ Data ³ 11.11.92 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
// validação do grupo de perguntas TPRTR001 para saber se o usuário selecionou um produto que corresponde a sua empresa configurada
// (impede de visualizar KARDEX de outros clientes)
User Function TPRTR01B (mvCodProd)
	local _lRet := .F.

	If (Valtype(mvCodProd) != "U")
		// valida se sigla do produto é valida
		_lRet := Ascan(___aPrtSigla, Substr(mvCodProd,1,4) ) != 0

		If !(_lRet)
			Alert("Código de produto inválido, em branco ou sem autorização para este cliente/usuário.")
		EndIf
	Endif
