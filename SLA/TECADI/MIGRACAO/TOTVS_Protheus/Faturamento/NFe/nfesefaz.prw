...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	// ESP. TECADI
	Private _aNfDevol     := {}
	Private _lMsgNFPedCli := .F.
	Private _lMsgNFAgrupa := .F.
	Private _lMsgNFDocCli := .F.
	Private _cPedCli      := ""
	Private _cDocCli      := ""
	Private _cAgrupaCli   := ""
	Private _lCodPrdCli   := .f.
	...
	// validação para identificar se cliente usa ou o não parametros adicionais na mensagem da NFe
	_lMsgNFAgrupa := U_FtWmsParam("WMS_NFE_MSG_AGRUPADORA","L",.f.,.f.,Nil, cClieFor, cLoja, Nil, Nil) // agrupadora
	_lMsgNFPedCli := U_FtWmsParam("WMS_NFE_MSG_PEDCLI","L",.f.,.f.,Nil, cClieFor, cLoja, Nil, Nil) // pedido do cliente
	_lMsgNFDocCli := U_FtWmsParam("WMS_NFE_MSG_DOCCLI","L",.f.,.f.,Nil, cClieFor, cLoja, Nil, Nil) // nota fiscal de venda final do cliente

	// define o uso do codigo do produto do cliente, removendo a SIGLA do inicio do codigo (não remover das variaveis, somente na montagem do XML)
	_lCodPrdCli := U_FtWmsParam("WMS_COD_PROD_CLIENTE_NF_RETORNO", "L", .f., .f., Nil, cClieFor, cLoja, Nil, Nil) // pedido do cliente
						...
						If lContinua
							...
							If !Empty((cAliasSD2)->D2_NFORI)
								...
								Else
									...
									// ESP. TECADI - INI
									If (aScan(_aNfDevol, {|x| (x[1]==SD2->D2_DOC).and.(x[2]==SD2->D2_SERIE).and.(x[3]==SF2->F2_EMISSAO) })==0)
										aAdd(_aNfDevol,{SD2->D2_DOC, SD2->D2_SERIE, SF2->F2_EMISSAO })
									EndIf
									// ESP. TECADI - FIM
							...
							// valido se o pedido e agrupadora do cliente já foram incluídos na mensagem da NFe
							// se o parâmetro de agrupadora estiver ativo
							If (_lMsgNFAgrupa)
								// valido se já foi informada e adiciono registros únicos
								If ( ! Alltrim(SC5->C5_ZAGRUPA) $ _cAgrupaCli )
									_cAgrupaCli += IIf(Empty(_cAgrupaCli),"","/ " ) + Alltrim(SC5->C5_ZAGRUPA)
								EndIf
							EndIf

							// se o parametro de pedido estiver ativo
							If (_lMsgNFPedCli)
								// valido se já foi informada e adiciono registros únicos
								If ( !  Alltrim(SC6->C6_PEDCLI) $ _cPedCli )
									_cPedCli += IIf(Empty(_cPedCli),"","/ " ) + Alltrim(SC6->C6_PEDCLI)
								EndIf
							EndIf

							// se o parametro de nota fiscal final do cliente estiver ativo
							If (_lMsgNFDocCli)
								// valido se já foi informada e adiciono registros únicos
								If ( !  Alltrim(SC5->C5_ZDOCCLI) $ _cDocCli )
									_cDocCli += IIf(Empty(_cDocCli),"","/ " ) + Alltrim(SC5->C5_ZDOCCLI)
								EndIf
							EndIf
							...
							//TECADI - 29.07.2019 - Luiz Fernando
							//Informações adicionais Sumitomo. (Fonte: TWMSA047)
							cInfAdic+= U_TWMA047N((cAliasSD2)->D2_FILIAL,(cAliasSD2)->D2_NFORI,(cAliasSD2)->D2_SERIORI,(cAliasSD2)->D2_ITEMORI,(cAliasSD2)->D2_CLIENTE,(cAliasSD2)->D2_LOJA,(cAliasSD2)->D2_COD) // ESP. TECADI
							//TECADI - Fim alteração.  
							...
							IIF((cAliasSD2)->(FieldPos("D2_GRPCST")) > 0 .and. !Empty((cAliasSD2)->D2_GRPCST),(cAliasSD2)->D2_GRPCST,IIF(SB1->(FieldPos("B1_GRPCST")) > 0 .and. !Empty(SB1->B1_GRPCST),SB1->B1_GRPCST, IIF(SF4->(FieldPos("F4_GRPCST")) > 0 .and. !Empty(SF4->F4_GRPCST),SF4->F4_GRPCST,"103"))),; //aProd[40] // ** ESP. TECADI
							...
							Alltrim((cAliasSD2)->D2_COD) == Alltrim(CD2->CD2_CODPRO)
								...
								Do Case
									...
									If !lConsig
										...
										SB1->B1_CLASSE,;
										0,;
										IIf(CD2->(FieldPos("CD2_GRPCST")) > 0 .and. !Empty(CD2->CD2_GRPCST),CD2->CD2_GRPCST,"103"),; //NT2015/002 // ** ESP. TECADI
										CD2->CD2_CST,;
										CD2->CD2_BC,;
										CD2->CD2_QTRIB,;
										CD2->CD2_PAUTA,;
										...
										If (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM) .And. !Empty(nValIPI) 
											aTail(aIPI) := {SB1->B1_SELOEN,SB1->B1_CLASSE,0,IIf(CD2->(FieldPos("CD2_GRPCST")) > 0  .and. !Empty(CD2->CD2_GRPCST),CD2->CD2_GRPCST,"103"),CD2->CD2_CST,0,0,CD2->CD2_PAUTA,0,0,CD2->CD2_MODBC,0} // ** ESP. TECADI
										EndIf
		...
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			Else				
				...
				SF1->F1_LOJA ==  (cAliasSD1)->D1_LOJA				
					...
					0,;		// [38]
					0,;		// [39]
					IIF((cAliasSD1)->(FieldPos("D1_GRPCST")) > 0 .and. !Empty((cAliasSD1)->D1_GRPCST),(cAliasSD1)->D1_GRPCST,IIF(SB1->(FieldPos("B1_GRPCST")) > 0 .and. !Empty(SB1->B1_GRPCST),SB1->B1_GRPCST, IIF(SF4->(FieldPos("F4_GRPCST")) > 0 .and. !Empty(SF4->F4_GRPCST),SF4->F4_GRPCST,"103"))),; //[40] // ** ESP. TECADI
					IIF(SB1->(FieldPos("B1_CEST"))<>0,SB1->B1_CEST,""),; //aprod[41] NT2015/003
					...
					(cAliasSD1)->D1_COD == CD2->CD2_CODPRO
						...
						Do Case
							...
							Case AllTrim(CD2->CD2_IMP) == "IPI"
							aTail(aIPI) := {SB1->B1_SELOEN,SB1->B1_CLASSE,0,IIf(CD2->(FieldPos("CD2_GRPCST")) > 0 .and. !Empty(CD2->CD2_GRPCST),CD2->CD2_GRPCST,"103"),CD2->CD2_CST,CD2->CD2_BC,CD2->CD2_QTRIB,CD2->CD2_PAUTA,CD2->CD2_ALIQ,CD2->CD2_VLTRIB,CD2->CD2_MODBC,CD2->CD2_PREDBC} // ** ESP. TECADI
							nValIPI := CD2->CD2_VLTRIB
							...
							If (((cAliasSD1)->D1_TIPO == "D" .and. !lEipiDev))  .Or. ((cAliasSD1)->D1_TIPO == "B" .And. lIpiBenef .and. !Empty(nValIPI))	
								...
								aTail(aIPI) := {SB1->B1_SELOEN,SB1->B1_CLASSE,0,IIf(CD2->(FieldPos("CD2_GRPCST")) > 0 .and. !Empty(CD2->CD2_GRPCST),CD2->CD2_GRPCST,"103"),CD2->CD2_CST,0,CD2->CD2_QTRIB,CD2->CD2_PAUTA,0,0,CD2->CD2_MODBC,CD2->CD2_PREDBC}	// ** ESP. TECADI											
							EndIf							
	...
	// ESP. TECADI - INI
	// mensagem das notas fiscais de devolucao
	If (Len(_aNfDevol)>0)
		cMensCli += " Retorno de Armazenagem da(s) sua(s) NF(s): "
		For nX := 1 to Len(_aNfDevol)
			// nota + serie + data
			cMensCli += AllTrim(_aNfDevol[nX,1])+;
			" Serie: "+AllTrim(_aNfDevol[nX,2])+;
			" Data: "+DtoC(_aNfDevol[nX,3])+" / "
		Next Nx
	EndIf

	If (_lMsgNFAgrupa).And.(! Empty(_cAgrupaCli))
		// incluo a mensagem
		cMensCli += " - Agrupador(as) do Cliente: " + _cAgrupaCli
	EndIf

	// se o parametro de pedido estiver ativo
	If (_lMsgNFPedCli).And.(! Empty(_cPedCli))
		// incluo a mensagem
		cMensCli += " - Pedido(s) do Cliente: " + _cPedCli
	EndIf

	// se o parametro de nota fiscal final estiver ativo
	If (_lMsgNFDocCli).And.(! Empty(_cDocCli))
		// incluo a mensagem
		cMensCli += " - Nota(s) fiscal(is) do Cliente: " + _cDocCli
	EndIf
	...
	IF lPe01Nfe     
		...
		aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,cTipo} // ** ESP. TECADI
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,aNota,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres,aAgrPis,aAgrCofins,nIcmsDif,aICMUFDest,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,cAmbiente,aIPIDevol,nvBCUFDest, aItemVinc,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aLote,cMensDifal)
	...
	// ESP. TECADI - INI
	// codigo produto do cliente
	local _cCodProd := aProd[02]
	// ESP. TECADI - FIM
	...
	// ESP. TECADI - INI
	If (_lCodPrdCli)
		_cCodProd := Posicione("SB1", 1, xFilial("SB1")+_cCodProd, "B1_CODCLI")
	EndIf

	// remocao do codigo de barras do tipo EAN
	aProd[03] := ""
	...
	cString += '<cProd>' + ConvType(_cCodProd) + '</cProd>' // ESP. TECADI
