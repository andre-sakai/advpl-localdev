...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local lNFeDesc  := .T. // GetNewPar("MV_NFEDESC",.F.) // ** ESP. TECADI
	...
	If cTipo == "1" .And. Empty(cMotCancela)
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				U_FtDscNfs("1"                        ,;
					...
					SC6->C6_DESCRI                     ,;
					(cAliasSD2)->D2_PRUNIT             ,;
					(cAliasSD2)->(D2_TOTAL + D2_DESCON) )

				// aglutina varios pedidos em uma unica nota?
				If (Empty(_cTmpPedido))
					_cTmpPedido := (cAliasSD2)->D2_PEDIDO
				EndIf

				// verifica se sao pedidos distintos na mesma nota
				If (_cTmpPedido != (cAliasSD2)->D2_PEDIDO)
					_lAglutNFS := .t.
				EndIf

				// armazena numeros dos pedidos do cliente
				If (lSC5).And.(SC5->(FieldPos("C5_ZPEDCLI")) > 0).And.(!Empty(SC5->C5_ZPEDCLI))
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				// atualiza a descricao completa dos servicos
				cDiscrNFSe := ""
				...
				If ( ! _lAglutNFS)
					cDiscrNFSe += If(Empty(cMensCli),"",CRLF+CRLF)+cMensCli
				EndIf

				// apaga mensagens especificas do cliente para nao aparecer no campo observacao
				cMensCli := ""
				...
				If (Len(_aRefPedCli) > 0)
					...
					cDiscrNFSe += CRLF+CRLF+"SEU(S) PEDIDO(S): "+_cRefPedCli
				EndIf
				// ** ESP. TECADI - FIM
				...
				cString	+= prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI, _cUfPrest ) // ** ESP. TECADI
...
static function prest()
	...
	if( type( "oSigamatX" ) == "U" )
		...
		cString	+= "<razao>" + convType(SM0->M0_NOMECOM,Len(AllTrim(SM0->M0_NOMECOM))) + "</razao>" // ** ESP. TECADI
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI, mvUfPrest ) // ** ESP. TECADI
	...
	default	cMunPrest	:= ""
	default	cMunPSIAFI	:= ""
	default mvUfPrest   := "" // ** ESP. TECADI

	aTabIBGE	:= spedTabIBGE()

	// ** ESP. TECADI - retira espacos
	cMunPrest := AllTrim(cMunPrest)

	If Len(alltrim(cMunPrest)) <= 5
		//nScan	:= aScan( aTabIBGE, { | x | x[1] == aDest[9] } )
		nScan	:= aScan( aTabIBGE, { | x | x[1] == mvUfPrest } ) // ** ESP. TECADI
		if nScan <= 0
			//nScan		:= aScan( aTabIBGE, { | x | x[4] == aDest[9] } )
			nScan		:= aScan( aTabIBGE, { | x | x[4] == mvUfPrest } ) // ** ESP. TECADI
...
Static Function tomador( aDest )
	...
	If !lSemTomador
		...
		Else
			...
			cString += '<notificatomador>2</notificatomador>' // ** ESP. TECADI
		EndIf
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)
	...
	for nX := 1 to len( aProd )
		...
		ElseIf !lNFeDesc
			...
			cString	+= "<discr>" + convType(cNatOper,Len(AllTrim(cNatOper)))+ cCargaTrb + "</discr>" // ** ESP. TECADI
		Else
			//cString	+= "<discr>" + AllTrim(cDiscrNFSe)+ cCargaTrb + "</discr>"
			cString	+= "<discr>" + convType(cDiscrNFSe,Len(AllTrim(cDiscrNFSe)))+ cCargaTrb + "</discr>" // ** ESP. TECADI
		EndIf
...
static function pagtos( aDupl,cCondPag )
	...
	If len( aDupl ) > 0
		...
		If ("00" == AllTrim(cCondPag)) .Or. ("00," == AllTrim(cCondPag)) // Pega do campo E4_COND
			cString += "<condpag>1</condpag>" // ** ESP. TECADI
		Else
			cString += "<condpag>3</condpag>" // ** ESP. TECADI
		Endif
