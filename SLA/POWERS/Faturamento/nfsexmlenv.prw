...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local cMenRET   	:= ""			//Alterado Vinicius 12/03/2014
	Local cMenDUPL   	:= ""			//Alterado Vinicius 12/03/2014
	Local lFirst 	  	:= .T.			//Alterado Vinicius 12/03/2014
	local _nTotal 		:= 0			// Alterado Tadeu
	Local nAliquota		:= 0 			// Alterado Tadeu
	Local nTributo 		:= 0			// Alterado Tadeu
	Local nTotTrib 		:= 0			// Alterado Tadeu 
	Local cPercTrib		:= 0			// Alterado Renata
	Local nTotNota 		:= 0			// Alterado Renata
	Local _cMensTrib	:= ""			// Alterado Tadeu
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			If !Empty(SF2->F2_DUPL)	
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSE1
						...
						SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_ORIGEM,E1_CSLL,E1_COFINS,E1_PIS,E1_PIS,E1_IRRF,E1_INSS,E1_ISS,E1_MOEDA,E1_CLIENTE,E1_LOJA, E1_DEPOSIT
						FROM %Table:SE1% SE1
						WHERE
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
					...
					//³Alterado por Vinicius/Jeyson(SMS) em 12/03/2014 para inclusão de mensagens complementares na NFS-e de Florianopolis³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				   //	cValor := Transform(((cAliasSE1)->E1_VALOR - (cAliasSE1)->E1_IRRF - (cAliasSE1)->E1_PIS - (cAliasSE1)->E1_COFINS - (cAliasSE1)->E1_CSLL - (cAliasSE1)->E1_INSS) - (cAliasSE1)->E1_ISS ,"@E 999,999,999.99")
				   cValor := Transform(((cAliasSE1)->E1_VALOR - (cAliasSE1)->E1_IRRF  - (cAliasSE1)->E1_PIS - (cAliasSE1)->E1_COFINS - (cAliasSE1)->E1_CSLL - (cAliasSE1)->E1_INSS)  ,"@E 999,999,999.99")
					If (cAliasSE1)->E1_DEPOSIT == '2'     //deposito em c.c
						cMenDUPL += "Valor: " + AllTrim(cValor) + " "
						cMenDUPL += "Banco Itau - Ag.: 0289 - C/c: 35625-5  "
					Else 
						If Alltrim((cAliasSE1)->E1_TIPO) $ "NF"
							If !Empty((cAliasSE1)->E1_PARCELA)
							   cMenDUPL += "Parcela: " + AllTrim((cAliasSE1)->E1_PARCELA) + " "
							EndIf
							cMenDUPL += "Valor: " + AllTrim(cValor) + " "
							cMenDUPL += "Vcto: " + DtoC((cAliasSE1)->E1_VENCORI) + " | "   
						endif												
	               EndIf
					//Fim alteração 
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//Inclusão Calculo Lei da Transparencia - Inicio
			
			_nTotal :=  (aProd[1][10] * aProd[1][09])			
			nAliquota:= GetAliqImp(aprod) 
			nTributo := ConvType((_nTotal  * nAliquota)/100,15,2)
			nTotTrib := Val(nTributo)
			
			If nTotTrib > 0
				nTotNota 	:= Val(ConvType(_nTotal,15,2))
				cPercTrib	:= ConvType((nTotTrib / nTotNota) * 100,15,2)
			EndIf
			
			cCargaTrb := "Valor Aproximado dos Tributos: R$ " +ConvType(nTotTrib,15,2)+" ("+cPercTrib+"%) Fonte: IBPT."	

			//Inclusão Calculo Lei da Transparencia - Final

						
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Alterado por Vinicius (SMS) em 12/03/2014 para inclusão de mensagens complementares na NFS-e de Florianopolis³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			//Colocando informações das retenções na descrição do serviço
			If (Select("QRY_RET") > 0)
				DbSelectArea("QRY_RET")
				DbCloseArea()
			EndIf

			cQryRET := "SELECT * "
			cQryRET += "  FROM " + retSqlName("SE1") + " SE1 "
			cQryRET += " WHERE SE1.D_E_L_E_T_ = ' ' "
			cQryRET += "   AND E1_FILIAL = '" + xFilial("SE1") + "' "
			cQryRET += "   AND E1_NUM = '" + SF2->F2_DOC + "' "
			cQryRET += "   AND E1_PREFIXO = '" + SF2->F2_PREFIXO + "' "
			cQryRET += "   AND E1_TIPO <> 'NF' "
			
			TcQuery	cQryRET NEW ALIAS "QRY_RET"
			
			While !QRY_RET->(EOF())
				If lFirst
					cVlrRET := Transform(QRY_RET->E1_VALOR,"@E 999,999,999.99")
				    cMenRET := SubStr(QRY_RET->E1_TIPO,1,2) + ": " + AllTrim(cVlrRET)
					lFirst := .F.
				Else
					cVlrRET := Transform(QRY_RET->E1_VALOR,"@E 999,999,999.99")
					cMenRET += ", " + SubStr(QRY_RET->E1_TIPO,1,2) + ": " + AllTrim(cVlrRET)
			...
			aProd[1,4] := aProd[1,4] + space(1) + cMenRET   
			
		  //	cMensCli := aProd[1,4]
			
			//Colocando informações das duplicadas das mensagens complementares
			If  !Empty(cMenDUPL)     //AllTrim(cCodMun) == "4205407" .And.
				cCargaTrb := cMenDUPL  +space(1) + cCargaTrb
			EndIf							
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf, cMunPrest, ADEST ) 
				...
				cString	+= infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe,  cCargaTrb, aProd)
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf, cMunPrest, ADEST ) 	
	...
	for nX := 1 to len( aProd )
		...
		*/			
		...
		if !lNFeDesc
			cString	+= "<discr>" + AllTrim(aProd[nX][4]) + "</discr>"
		Else
			cString	+= "<discr>" + AllTrim(cDiscrNFSe)+ cCargaTrb + "</discr>"
		EndIf 
        ...
        */
        ...
        If cCodmun == "4205407"
			If cCodmun == Iif( Empty(cMunPrest) .Or. Len(AllTrim(cMunPrest)) < 6, UFCodIBGE2(aDest[09]) +aDest[07] ,cMunPrest )   
		   		aProd[Nx][33] := "9201"
				cString += '<cfps>'+SubStr(aProd[Nx][33],1,3)+'1</cfps>'
			ElseIf AllTrim(SM0->M0_ESTCOB) == AllTrim(aDest[09])   
				aProd[Nx][33] := "9202"
				cString += '<cfps>'+aProd[Nx][33] +'</cfps>'
			Else
				aProd[Nx][33] := "9203"
				cString += '<cfps>'+aProd[Nx][33]+'</cfps>'
			EndIf
...
/*/
//-----------------------------------------------------------------------
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe , cCargaTrb, aProd)
	...
	If !lNFeDesc
	   //	cString	+= "<descricao>" + cMensCli + space( 1 ) + cMensFis + "</descricao>"
	   	cString	+= "<descricao>"  + cMensCli + space( 1 ) + cMensFis +  space( 1 ) + cCargaTrb + "</descricao>"
	Else
		cString	+= "<descricao>" + Alltrim(cDescrNFSe) + "</descricao>"
	EndIf
	
   //	cString += "<observacao>" + cMensCli + space( 1 ) + cMensFis + "</observacao>"
   	cString += "<observacao>" + cMensCli + space( 1 ) + cMensFis +  space( 1 ) + cCargaTrb + "</observacao>"
	cString	+= "</infcompl>"
...
IF ALLTRIM(SM0->M0_CODIGO) == '01'
	if ( EL0->(dbSeek(xFilial("EL0")+aprod[1][5])) )	
		nAliqImp := EL0->EL0_ALIQIM
	else
		nAliqImp := aprod[1][23]
	endif
ELSE
	if ( EL0->(dbSeek(xFilial("EL0")+aprod[1][2])) )	
		nAliqImp := EL0->EL0_ALIQIM
	else
		nAliqImp := aprod[1][23]
	endif
