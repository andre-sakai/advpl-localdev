...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				EndIf 
					...
					// Especifico Arteplas -> Inicio do tratamento da placa do veiculo no Pedido de Vendas

					DbSelectArea("SC5")
                	    DbSetOrder(1)
                    	If DbSeek(xFilial("SC5")+SD2->D2_PEDIDO)
                    cPlaca   := StrTran(SC5->C5_PLACA,"-","")
                    cUFPlaca := SC5->C5_UFPLACA
                    If !Empty(cPlaca) .And. !Empty(cUFPlaca)
                         aAdd(aVeiculo, cPlaca ) 	// Placa da Transportadora
                         aAdd(aVeiculo, cUFPlaca ) 	// UF da Transportadora
                         aAdd(aVeiculo, "" )		// RNTC
    	    	            Endif
    	            EndIf    
					// Fim Placa Arteplas
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//ARTEPLAS - 17/07/18 - Num Pedido para Observações
						
						If aScan(aNPed,{|x| Alltrim(x[1])==Alltrim(SC5->C5_NUM)})==0
							aAdd(aNPed,{SC5->C5_NUM})
						EndIf         
		            	...
		            	// Arteplas - Codigo do Produto na Amarração Produto x Cliente
						dbSelectArea("SA7")
						dbSetOrder(1)
						dbGoTop()
				...
				//ARTEPLAS - 17/07/18 - Número no Nosso Pedido no CMensCli
			    cNPed	:= ""
				For n:=1 to len(aNPed)
					cNPed += If(!Empty(aNPed[n,1]),aNPed[n,1]+"/","")
				Next 
	
				cNPed	:= Substr(cNPed,1,Len(cNPed)-1)
				cMensPed := "  Nosso Pedido: "+cNPed+" "
				If !AllTrim(cMensPed) $ cMensCli
					cMensCli += AllTrim(cMensPed)
				EndIf        
